...
const getMinAge = (data) => {
	return data.reduce((min, passenger) => {
		if (passenger.fields.age < min) {
...
const getMaxAge = (data) => {
	return data.reduce((max, passenger) => {
		if (passenger.fields.age > max) {
...
const getAverageFare = (data) => {
	...
	return filteredData.reduce((total, passenger) => {
		total += passenger.fields.fare
		return total
...
const getMedianFare = (data) => {
	const filteredData = data.filter(passenger => passenger.fields.fare !== undefined)
	const sortedData = filteredData.sort((a, b) => a.fields.fare - b.fields.fare)
	if (sortedData.length % 2 === 0) {
		return (sortedData[sortedData.length / 2].fields.fare + sortedData[(sortedData.length / 2) - 1].fields.fare) / 2
	} else {
		return sortedData[Math.floor(sortedData.length / 2)].fields.fare
	}
}
...
const getMedianAge = (data) => {
	const filteredData = data.filter(passenger => passenger.fields.age !== undefined)
	const sortedData = filteredData.sort((a, b) => a.fields.age - b.fields.age)
	if (sortedData.length % 2 === 0) {
		return (sortedData[sortedData.length / 2].fields.age + sortedData[(sortedData.length / 2) - 1].fields.age) / 2
	} else {
		return sortedData[Math.floor(sortedData.length / 2)].fields.age
	}
}
