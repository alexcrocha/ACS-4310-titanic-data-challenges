...
const sumAllProperty = (data, property) => {
	return data.reduce((total, passenger) => {
		if (passenger.fields[property] !== undefined) {
			total += passenger.fields[property]
		}
		return total
	}, 0)
}
...
const countAllProperty = (data, property) => {
	return data.map(passenger => passenger.fields[property]).reduce((acc, value) => {
...
const makeHistogram = (data, property, step) => {
	...
	const bins = filteredData.map(passenger => passenger.fields[property]).reduce((acc, value) => {
		acc[Math.floor(value / step)] = (acc[Math.floor(value / step)] || 0) + 1
		return acc
	}, [])
	return Array.from(bins, value => value || 0)
}
